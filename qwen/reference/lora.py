...
from vila3r.model import *
from llava.train.rope2d import get_rope_index_25
...
class ModelArguments:
    ...
    vggt_model: Optional[str] = field(default=None)
...
class DataArguments:
    ...
    video_max_frame_pixels: Optional[int] = field(default=384)
    max_image_size: int = 504
...
def find_all_linear_names(model):
    ...
    multimodal_keywords = ['visual', 'recon_feature_processor', 'simple_fusion', 'vggt']
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):  # 对于 pretraining 来说我们只需要保存 projection layer 
        # Only save Adapter
        keys_to_match = ['mm_projector', 'video_tower', 'reecon_feature_processor', 'simple_fusion']
...
class LazySupervisedDataset(Dataset):
    ...
    def process_sampled_frames(self, sampled_image_list):
        ...
        resized_images = [resize_with_fixed_width_and_rounded_height(img, target_width=self.data_args.max_image_size) for img in images]

        duplicated_images = []
        for img in resized_images:
            duplicated_images.extend([img, img.copy()]) 
            
        # # 计算伪fps和video_length
        # interval = getattr(self.data_args, "base_interval", 4)
        # video_length = interval * len(images)
        # fps = len(images) / video_length  # 即 1/interval
        ...
        video_processed = processor.preprocess(
            images=None, videos=duplicated_images, return_tensors="pt"
        )
    ...
    def _load_and_cache_vggt_data(self, vggt_tensor_file):
        """Helper function to load and cache scan data."""
        if vggt_tensor_file in self.cache:
            # If the data is already cached, return it from the cache
            return self.cache[vggt_tensor_file]

        self.cache[vggt_tensor_file] = load_file(vggt_tensor_file)["feature"]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        sample_info_file = os.path.join(self.data_args.video_folder, "sample_32_frames", f"{video_file}.txt")
        sampled_frames = read_frame_paths_from_txt(sample_info_file)

        video, grid_thw, second_per_grid_ts, vggt_images = self.process_sampled_frames(sampled_frames)
        video = [video]
        ...
        # load vggt intermediate feature
        # vggt_tensor_file = os.path.join(self.data_args.video_folder, "vggt_output", f"{video_file}_avg_tensor.safetensors")
        # vggt_tensor = copy.deepcopy(self._load_and_cache_vggt_data(vggt_tensor_file))
        
        data_dict["vggt_images"] = vggt_images
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        batch["vggt_images"] = torch.stack([x["vggt_images"] for x in instances], dim=0)
        return batch
...
def load_model(model_args, training_args, bnb_model_from_pretrained_args):
    attn_implementation="flash_attention_2"
    
    if model_args.model_type == "latent":
        model = Vila3rOnlineVggtQwen2_5_VLForConditionalGeneration.from_pretrained(
    ...
    elif model_args.model_type == "world_coord":
        model = Vila3rWorldCoordQwen2_5_VLForConditionalGeneration.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            attn_implementation=attn_implementation,
            torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
        ...
        raise ValueError("the model_type should be latent or world_coord")
    
    _state_dict = torch.load(model_args.vggt_model, map_location="cpu")
    model.vggt.load_state_dict(_state_dict, strict=False)
    target_dtype = next(model.parameters()).dtype  # 使用主模型 dtype
    model.vggt = model.vggt.to(dtype=target_dtype)
...
def train(attn_implementation=None):
    ...
    # 加载模型
    model = load_model(model_args, training_args, bnb_model_from_pretrained_args)
    model.config.use_cache = False

    tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path)
    processor = AutoProcessor.from_pretrained(model_args.model_name_or_path).image_processor
    ...
    # Enable Training lm_head and embed_tokens (to be checked)
    ...
    model.model.recon_feature_processor.requires_grad_(True)
    model.model.vggt.requires_grad_(False)
    model.model.visual.merger.requires_grad_(True)
    # model.model.simple_fusion.requires_grad_(True)
