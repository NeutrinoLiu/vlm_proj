...
class NuScenes:
    ...
    def get_sample_data_verbose(self, sample_data_token: str,
        ...
        if selected_anntokens is not None:
            annos = selected_anntokens
            boxes = list(map(self.get_box, selected_anntokens))
        else:
            annos = self.get('sample', sd_record['sample_token'])['anns']
        ...
        for box in boxes:
            ...
            if sensor_record['modality'] == 'camera' and not \
                ...
                box_list_details.append(None)
                continue

            box_list.append(box)
            box_list_details.append(box.copy())
        
        box_list_details = dict(zip(annos, box_list_details))

        return data_path, box_list, cam_intrinsic, box_list_details

    def get_sample_data(self, *args, **kwargs):
        """
        Returns the data path as well as all annotations related to that sample_data.
        Note that the boxes are transformed into the current sensor's coordinate frame.
        :param args: Arguments to pass to get_sample_data_verbose.
        :param kwargs: Keyword arguments to pass to get_sample_data_verbose.
        :return: (data_path, boxes, camera_intrinsic <np.array: 3, 3>)
        """
        return self.get_sample_data_verbose(*args, **kwargs)[:3]
...
class NuScenesExplorer:
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            data_path, boxes, camera_intrinsic, boxes_details = self.nusc.get_sample_data_verbose(sample_data_token,
            ...
            if with_anns:
                for box in boxes:
                    c = np.array(self.get_color(box.name)) / 255.0
                    corners = box.render(ax, view=camera_intrinsic, normalize=True, colors=(c, c, c))
                    boxes_details[box.token] = {
                        'box': box,
                        'corners': corners,
        ...
        return boxes_details if sensor_modality == 'camera' else None
